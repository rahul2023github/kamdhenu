...
class NewsletterSubscription extends NewsletterModule {
    ...
    function get_form_hidden_fields($attrs) {
        ...
        if (isset($attrs['welcome_email'])) {
            $e = $this->get_email($attrs['welcome_email']);
            if ($e->type === 'welcome') {
                $b .= '<input type="hidden" name="welcome_email" value="' . esc_attr($e->id . '-' . $e->token) . '">';
            } else {
...
//                        $a .= $k . '="' . $v . '" ';
//                    }
                    $message = str_replace('{subscription_form}', '[newsletter_form ' . $a . ']', $message);
                }
            }
        }
